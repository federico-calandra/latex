...
Si cercano soluzioni delle EdM in assenza di sorgenti, della forma $\mathbf E = \mathbf E(x,t)$ e $\mathbf B = \mathbf B(x,t)$ cioè che assumono valori costanti in piani $\perp$ all'asse $x$. Con queste ipotesi le EdM si semplificano e si trova: $E_x = 0 \quad B_x = 0 \quad \square E_\alpha = 0 \quad \square B_\alpha = 0$ con $\alpha = y,z$. Il campo elettromagnetico si propaga lungo l'asse $x$ sotto forma di onde piane trasversali con velocità $v = 1/\sqrt{\epsilon\mu}$; si può dimostrare che $v < c$ in ogni caso. Le precedenti si possono risolvere e individuano $\mathbf E = E_y(x-vt) \mathbf u_y + E_z(x-vt) \mathbf u_z$ e $\mathbf B = B_y(x-vt) \mathbf u_y + B_z(x-vt) \mathbf u_z$

\subsubsection{Polarizzazione}
Fra $\mathbf E$ e $\mathbf B$ esistono delle relazioni geometriche


\subsubsection{Teorema di Poynting}

\subsubsection{Equazione di D'Alambert}
Scrivo l'equazione del moto di un tratto $\de l$ di corda ideale (omogenmea, flessibile, inestensibile, \ldots) in direzione della sua oscillazione. Se chiamo $\xi$ lo spostamento del tratto infinitesimo allora $[0,L] \times [0,T] \ni (x,t)  \mapsto \xi(x,t)$ individua il moto della corda. La forza lungo $y$ è $F_y = T(\sin\alpha_2 - \sin\alpha_1) \approx T[(\xi_x(x_2) - \xi_x(x_1)] \approx T \xi_{xx}(\frac{x_1+x_2}2) \de x \approx T \xi_{xx} (\frac{x_1+x_2}2) \de l$. Quindi l'equazione del moto è $\rho \de l \xi_{tt} = T \xi_{xx} \de l$. Si definisce la velocità di fase $v = \sqrt{T/\rho}$ quindi $\xi_{tt} = v \xi_{xx}$. 
\end{document}
