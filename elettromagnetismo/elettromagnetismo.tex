...
Le equazioni di Maxwell sono state compiutamente definite (1865)
	...
	\section{Campi elettrici e magnetici nel vuoto}
...
\subsubsection{Moto in un campo elettrostatico}
L'equazione del moto è $q \mathbf E(\mathbf r) = m \ddot{\mathbf r}$. Se $\mathbf E$ è uniforme ($\mathbf E \parallel \mathbf x$) allora $\ddot x = \frac{qE}m$, cioè l'accelerazione è costante.
...
\subsubsection{Moto in un campo magnetostatico}
...
\begin{tabular}{c|llll}
	...
	IV & $\Gamma(\mathbf B) = \Phi(\mu_0 \mathbf J)$ & $\oint \mathbf B(\mathbf r) \cdot \de\mathbf s = \mu_0 \int \mathbf J(\mathbf r) \cdot \mathbf n \de\varsigma$ & $\nabla \times \mathbf B(\mathbf r) = \mu_0 \mathbf J(\mathbf r)$
\end{tabular}\\
Campo elettrico e campo magnetico sono entità separate



	\section{Campi elettrici e magnetici nella materia}
...
\subsubsection{Equazioni delle onde elettromagnetiche}
Si cercano soluzioni delle EdM in assenza di sorgenti, della forma $\mathbf E = \mathbf E(x,t)$ e $\mathbf B = \mathbf B(x,t)$ cioè che assumono valori costanti in piani $\perp$ all'asse $x$. Con queste ipotesi le EdM si semplificano ($\alpha = y,z$):\\
$\begin{cases} I \Rightarrow E_x = 0\\
II \Rightarrow B_x = 0\\
III \Rightarrow \square E_\alpha = 0\\
IV \Rightarrow \square B_\alpha = 0 \end{cases}$\\
Il campo elettromagnetico si propaga lungo l'asse $x$ sotto forma di onde piane trasversali con velocità $v = 1/\sqrt{\epsilon\mu}$; si può dimostrare che $v < c$ in ogni caso. Le precedenti si possono risolvere e individuano $\mathbf E = E_y(u) \mathbf u_y + E_z(u) \mathbf u_z$ e $\mathbf B = B_y(u) \mathbf u_y + B_z(u) \mathbf u_z$, dove $u = x-vt$.

\subsubsection{Relazioni fra i campi}
...
Vedi Mazzoldi Cap 13

\subsubsection{Teorema di Poynting}
La diminuzione di energia elettromagnetica è dovuta al flusso del vettore di Poynting e alla potenza trasferita dal campo alle cariche. La densità di energia del campo in mezzi isotropi lineari è $u = \frac12(\mathbf E \cdot \epsilon\mathbf E + \mathbf B \cdot \mathbf B/\mu$). La potenza trasferita ad una distribuzione continua è $\fracd{E_{mecc}}{t} = \int \mathbf E \cdot \mathbf J \de\upsilon$. Con la III e IV equazione, l'identità vettoriale DPV si può riscrivere $P = -\int (\dive(\mathbf E\times\mathbf H) + \mathbf E \cdot \partial_t\mathbf D + \mathbf B \cdot \partial_t \mathbf H)\de\upsilon$. Si riconosce $\partial_t u$ negli ultimi due termini del rhs, quindi $\int (\mathbf E \cdot \mathbf J)\de\upsilon = -\int(\dive(\mathbf E \times \mathbf H) + \partial_t u)\de\upsilon$. La forma locale $-\partial_t u = \dive\mathbf S - \mathbf E \cdot \mathbf J$ mette meglio in evidenza la legge di bilancio, essendo $\mathbf S = \mathbf E \times H$ il vettore di Poynting che descrive il flusso dell'energia trasportata dall'onda elettromagnetica.
...
\subsubsection{Pacchetto d'onda e indeterminazione}
Nella realtà ogni onda ha durata finita, cioè si parla di pacchetto d'onda che ha lunghezza $\Delta x = N \lambda$ e durata $\Delta t = N T$, dove $N$ è il numero di oscillazioni contenute nel pacchetto. In questo modo le quantità $k = 2\pi/\lambda= 2\pi N/\Delta x$ e $\omega = 2\pi/T = 2\pi N/\Delta t$ hanno significato più chiaro. Se $N$ non è precisamente determinato, cioèho indeterminazione $\Delta N$ sul numero di oscillazioni del pacchetto d'onda, allora avrò le indeterminazione per $\Delta k =  2\pi \Delta N/\Delta x$ e $\Delta\omega = 2\pi\Delta N/\Delta t$, e quindi anche sulla frequenza. Minore è a durata del pacchetto e maggiori sono le indeterminazioni.
