...
\subsubsection{Forza magnetica su elemento di corrente}
Su una particella di carica $q$ in moto con velocità $\mathbf v$ in un campo magnetico $\mathbf B$ è $\mathbf F = q\mathbf v \times \mathbf B$. In un conduttore avrò $n$ cariche per unità di volume in moto con velocità media $\mathbf v_{der}$ quindi 
...
$\Phi(\mathbf E) = Q_{int}/\epsilon \;\;\Longrightarrow\;\; \iint \mathbf E \cdot \mathbf n\de\varsigma = \frac 1\epsilon \iiint \rho\,\de\upsilon \;\;\Longrightarrow\;\; \dive\mathbf E = \rho/\epsilon$\\
definizione di flusso, teorema della divergenza

\subsubsection{II equazione - Inesistenza di monopoli magnetici}
$\Phi(\mathbf B) = 0 \;\;\Longrightarrow\;\; \iint \mathbf B \cdot \mathbf n\de\varsigma = 0 \;\;\Longrightarrow\;\; \dive\mathbf B = 0$\\
definizione di flusso, teorema della divergenza

\subsubsection{III equazione - Conservatività del campo elettrostatico}
$\Gamma(\mathbf E) = 0 \;\;\Longrightarrow\;\; \oint \mathbf E \cdot \de\mathbf s = 0 \;\;\Longrightarrow\;\; \rot\mathbf E = 0$\\
definizione di circuitazione, teorema del rotore

\subsubsection{IV equazione - Teorema di Ampère - Sorgenti del campo magnetico}
$\Gamma(\mathbf B) = \mu_0 \sum_k m_k i_k \;\;\Longrightarrow\;\; \oint \mathbf B \cdot \de\mathbf s = \mu_0 \iint \mathbf J \cdot \mathbf n\de\varsigma \;\;\Longrightarrow\;\; \rot\mathbf B  = \mu_0 \mathbf J$\\
...
\subsubsection{I equazione}
$\Phi(\mathbf D) = Q_{lib} \;\;\Longrightarrow\;\; \iint \mathbf D \cdot \mathbf n\de\varsigma = \iiint \rho_{lib} \,\de\upsilon \;\;\Longrightarrow\;\; \dive\mathbf D = \rho_{lib}$\\
definizione di flusso, teorema della divergenza

\subsubsection{II equazione}
$\Phi(\mathbf B) = 0 \;\;\Longrightarrow\;\; \iint \mathbf B \cdot \mathbf n\de\varsigma = 0 \;\;\Longrightarrow\;\; \dive\mathbf B = 0$\\
definizione di flusso, teorema della divergenza

\subsubsection{III equazione}
$\Gamma(\mathbf E) = 0 \;\;\Longrightarrow\;\; \oint \mathbf E \cdot \de\mathbf s = 0 \;\;\Longrightarrow\;\; \rot\mathbf E = 0$\\
definizione di circuitazione, teorema del rotore
