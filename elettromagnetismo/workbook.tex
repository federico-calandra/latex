...
Il teorema di Gauss per $\mathbf E = \mathbf E_{lib} + \mathbf E_{pol}$ fornisce $\int \dive\mathbf E \de\upsilon = (Q_{lib} + Q_{pol})/\varepsilon$. Sapendo la relazione fra $Q_{pol}$ e $\rho_{pol}$ si ottiene $\frac1\varepsilon \int \dive(\varepsilon\mathbf E +  \mathbf P)\de\upsilon = \frac 1\varepsilon \int \rho_{lib} \de\upsilon$ da cui $\dive\mathbf D = \rho_{lib}$.
...
La spira ha raggio $R$, è posta nel piano $Oxy$ e conduce la corrente $i$. Si vuole calcolare il campo sull'asse della spira. Gli ingredienti della legge di Biot-Savart sono: $\mathbf r = z\mathbf k \quad \mathbf r^\prime= R \mathbf u_\rho \quad \de\mathbf l^\prime = \de l \mathbf u_{\phi}\quad |\mathbf r - \mathbf r^\prime|^2 = z^2 + R^2$
...
\subsubsection{IV equazione - Teorema di Ampère}
...
definizione di circuitazione, densità di corrente, teorema del rotore
