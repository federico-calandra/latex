...
\subsubsection{La legge di Coulomb definisce una forza conservativa}
Nella definizione di campo elettrico si può usare l'identità $\nabla(-\frac1{|\mathbf r-\mathbf r^\prime|}) = \frac{\mathbf r-\mathbf r^\prime}{|\mathbf r-\mathbf r^\prime|^3}$ per definire $\mathbf E$ in termini del gradiente di una funzione (potenziale elettrostatico). La II equazione di Maxwell segue dall'identità $\rot(\nabla{A})=0$ dove $A$ è il potenziale elettrostatico definito sopra.
...
vedi Jackson, ma passaggio 5.16 non mi torna. Quindi $\mathbf A(\mathbf r) = \oint \frac{\mathbf J(\mathbf r^\prime) \de\upsilon^\prime}{|\mathbf r - \mathbf r^\prime|}$. Il calcolo $\rot\mathbf B = \rot(\rot\mathbf A)$ con identità vettoriale introduce un termine con $\nabla^2$

\subsubsection{Potenziale magnetico scalare}
...
Dalla precedente con $z \gg R$ risulta $\mathbf B(z) \simeq \frac{\mu_0 i}{2} \frac{R^2}{z^3} \mathbf u_z$. Posso reinterpretare questo risultato considerando che $A = \pi R^2$ e definendo (analogamente al dipolo elettrico) il momento magnetico della spira $\mathbf m = i A \mathbf n$. In questo caso $\mathbf n = \mathbf u_z$ quindi $\mathbf B(z \gg R) \simeq \frac{\mu_0}{2\pi} \frac{\mathbf m}{z^3}$.
...
$\begin{cases}
	...
	\nabla \times \mathbf E + \fracp{\mathbf B}t = 0\\
	\nabla \times \mathbf B - \mu_0\varepsilon_0 \fracp{\mathbf E}t = 0 
\end{cases}$
