...
\section{Elettrostatica e I equazione di Maxwell}	
	\subsection{Forza elettrica tra due cariche}
	...
	\subsection{Campo elettrico distribuzione mista}
	$\mathbf E(\mathbf r)=\frac1{4\pi\varepsilon}\left[\sum_{i=1}^N\frac{q_i(\mathbf r-\mathbf r_i)}{\|\mathbf r-\mathbf r_i\|^3}+\int_\Upsilon\frac{\varrho(\mathbf r \, ')(\mathbf r-\mathbf r\,')}{\| \mathbf r-\mathbf r \, '\|^3} \, \mathrm{d}\upsilon'\right]$\\
	$V(\mathbf r)=\frac1{4\pi\varepsilon}\left[\sum_{i=1}^N\frac{q_i}{\|\mathbf r-\mathbf r_i\|}+\int_\Upsilon\!\!\frac{\varrho(\mathbf r')}{\|\mathbf r-\mathbf r'\|}\mathrm{d}\upsilon'\right]$
	\subsection{Teorema di Gauss e I equazione di Maxwell}
	$\Phi_{\partial \mathcal G}(\mathbf E) = \frac 1{\varepsilon} \int_\mathcal G \varrho(\mathbf r \,') \, d\upsilon'\;\;\Rightarrow\;\;\mathrm{div}\,\mathbf E=\frac 1{\varepsilon}\,\varrho$
 	\subsection{Equazione di Poisson e di Laplace}
 	$\mathbf E=-\vec\nabla V\;\Rightarrow\;\nabla^2V(\mathbf r)=\frac {1}{\varepsilon}\varrho(\mathbf r)\qquad\rho(\mathbf r)\equiv 0\;\Rightarrow\;\nabla^2V(\mathbf r)=0$
	\subsection{Relazioni per l'elettrostatica nel vuoto}
	$\mathbf E_0=\varepsilon_r\mathbf E\qquad V_0=\varepsilon_r V\qquad \varepsilon=\varepsilon_r\,\varepsilon_0\qquad\varepsilon_r>1$
...
\section{Aspetti energetici e III equazione di maxwell}
	\subsection{Lavoro, potenziale e energia elettrostatica}
 	$\de L=\mathbf F\cdot\mathrm d\mathbf s=q\mathbf E\cdot\de\mathbf s=q\,(-\vec\nabla V)\cdot\de\mathbf s \qquad\qquad V_O(P):=\int_P^O\mathbf E\cdot\de\mathbf s$\\
 	$L_{AB}=q\int_A^B\!\mathbf E \cdot\de\mathbf s=-q\int_A^B \vec\nabla V\cdot\de\mathbf s=-q\int_A^B\de V=q(V(A)-V(B))$\\
 	$\mathbf E = -\vec\nabla V \Rightarrow L_{AB} = \Delta U = q\Delta V \Rightarrow U=q\,V$\\
 	\ldots energia di un sistema di cariche $\ldots$
 	$\Delta T = L_{AB} \Rightarrow \frac12 m(v_B^2-v_A^2) = q(V_A - V_B)$
 	
	\subsection{Conservatività e III equaz. di Maxwell stazionaria}	
	$\oint\!\mathbf F\cdot\mathrm{d}\mathbf s=q\oint\!\mathbf E\cdot\mathrm{d}\mathbf s=0\;\;\Rightarrow\;\;\mathrm{rot}\,\mathbf E=\vec 0$
	\subsection{Sovrapposizione lineare}
	$\mathbf E=\sum\mathbf E_i=\sum (-\vec\nabla V_i)=-\vec\nabla(\sum V_i)=-\vec\nabla V$	
	\subsection{Energia del campo elettrostatico}
	\subsubsection{Sistema di cariche discrete}
	\begin{tabular}{@{}lll}
	...
	\qquad\qquad$V_i=\sum_j \frac{q_j}{4\pi\varepsilon r_{ij}}$ potenziale generato in $P_i$ dalle altre cariche
	\subsubsection{Sistema di cariche distribuite}
	$U=\frac 12 \int_\Upsilon\varrho V\,\de\upsilon=\frac \varepsilon 2 \int_\Upsilon V\,\mathrm{div}\mathbf E\,\de\upsilon=\frac \varepsilon 2\int_\Upsilon[\mathrm{div}(V\mathbf E)+\|\mathbf E\|^2]\,\de\upsilon=$\\$\!\!\!=\frac \varepsilon 2\int_{\partial\Upsilon}V\,\mathbf E\cdot\mathbf n\,\mathrm dS+\frac \varepsilon 2\int_\Upsilon \|\mathbf E\|^2\,\de\upsilon\quad\underset{\Upsilon\to\mathbb{E}^3}{\lim}U=\int_{\mathbb{E}^3}\!\frac{\varepsilon \|\mathbf E\|^2}{2}\,\de\upsilon=\int_{\mathbb{E}^3}\!u\,\de\upsilon$

\vfill\null
\columnbreak	

\section{Capacità e condensatori}
	...
	incremento dovuto a traferimento di carica\quad$\mathrm dU=\frac{q}{C}\mathrm dq$\\
...
\section{Correnti elettriche stazionarie}
	...
	$\mathbf J=Nq\,\mathbf v_\mathrm{d}=\varrho\,\mathbf v_\de\quad\mathbf v_\mathrm{d}\parallel\mathbf E\qquad\mathrm dq=Nq\,\mathbf v_\mathrm{d}\cdot\mathbf n\,\mathrm dS\,\mathrm dt$\\
	$\mathrm dI=\frac{\mathrm dq}{\mathrm dt}=\mathbf J\cdot\mathbf n\,\mathrm dS\;\;\Rightarrow\;\;I(\Sigma)=\int_\Sigma\mathbf J\cdot\mathbf n\,\mathrm dS$
	...
	$\frac{\de}{\mathrm dt}\int_\Upsilon\varrho\,\de\upsilon=\int_{\partial\Upsilon}\mathbf J\cdot(-\mathbf n)\,\mathrm dS\qquad \frac{\partial\varrho}{\partial t}+\mathrm{div}\mathbf J=0$\\\smallskip
   
	per un nodo di $n$ conduttori\quad$\sum_i^{\mathrm{(alg.)}}I_i=0$
	
	\subsection{Circuitazione del c.e. e II legge di Kirchhoff}
	$\mathbf E=\mathbf E^\mathrm s+\mathbf E^\mathrm m+\mathbf E^\mathrm c=\rho\mathbf J\;\;\Rightarrow\;\;\sum_i^{\mathrm{(alg.)}}{\mathcal E_i}^\mathrm m+\sum_i^{\mathrm{(alg.)}}{\mathcal E_i}^\mathrm c=R_\mathrm{tot}I$ 	
 	
 	\subsection{Resistività}
   	$R(T)=\rho(T)\frac LS\qquad \rho(T)=r(\mathrm{0^\circ C})(1+\alpha T)\quad\alpha$ dipende dal materiale
	
\section{Magnetostatica nel vuoto}
	\subsection{Legge di Biot-Savart}
	\begin{tabular}{@{}ll}
	conduttore filiforme&conduttore generico\\
	$\mathbf B_0(\mathbf r)=\frac{\mu_0}{4\pi} \oint\frac{I\mathrm d \mathbf l^{\,\prime}\times(\mathbf r-\mathbf r^{\,\prime})}{\|\mathbf r-\mathbf r^{\,\prime}\|^3}$&$\de\mathbf B_0(\mathbf r)=\frac{\mu_0}{4\pi}\frac{\mathbf J(\mathbf r^{\,\prime})\times(\mathbf r-\mathbf r^{\,\prime})}{\|\mathbf r-\mathbf r^{\,\prime}\|^3}$\end{tabular}
	
	\subsection{Flusso di induzione e II equazione di Maxwell}
	$\Phi_\Sigma(\mathbf B_0)=\int_\Sigma\mathbf B_0\cdot\mathbf n\,\mathrm dS=0\;\;\Rightarrow\;\;\mathrm{div}\,\mathbf B_0=0$
	
	\subsection{Th. di Ampère e IV equaz. di Maxwell stazionaria}
	$\oint_\gamma\mathbf B_0\cdot\mathrm d\mathbf s=\mu_0\underset{\mathrm{alg}}{\sum}m_iI_i(\gamma)\;\;\Rightarrow\;\;\mathrm{rot}\mathbf B_0=\mu_0\mathbf J$\\
	$m_i$ grado di concatenazione di $I_i$
	
	\subsection{Forza magnetica}
	\begin{tabular}{@{}ll}
	conduttore filiforme&conduttore generico\\
	$\de\mathbf F=I\mathrm d\mathbf l\times\mathbf B_0$&$\de\mathbf F=\mathbf J\,\de\upsilon\times\mathbf B_0$\\
	$\de\mathbf M=\mathbf r\times(I\mathrm d\mathbf l\times\mathbf B_0)$&$\de\mathbf M=\mathbf r\times(\mathbf J\,\de\upsilon\times\mathbf B_0)$\end{tabular}
...
\section{Elettromagnetismo non stazionario}
	...
	$f_\mathrm{ind}=-\frac{\de\Phi_\gamma(\mathbf B)}{\mathrm dt}\qquad f_\mathrm{ind}=\oint_\gamma\mathbf E_\mathrm{ind}\cdot\mathrm d\mathbf s\quad \mathbf E_\mathrm{ind}=\mathbf E+\mathbf v_\mathrm{tr}\times\mathbf B$\\
	se $\mathbf v_\mathrm{tr}=\vec 0$ (circuito fermo) allora $\mathrm{rot}\,\mathbf E=-\frac{\partial\mathbf B}{\partial t}$
	...
	\subsection{Autoinduzione}
		...
		$\Phi_\gamma(\mathbf B_0)=I\Big(\frac{\mu_0}{4\pi}\int_{\Sigma(\gamma)}\oint_\gamma\frac{\de\mathbf l\times\Delta\mathbf r}{\|\Delta\mathbf r\|^3}\cdot\mathbf n\,\de\mathbf s\Big)\qquad L=\frac{\Phi_\gamma(\mathbf B_0)}{I}\qquad f_\mathrm a=-L\frac{\mathrm dI}{\mathrm dt}$
		\subsubsection{energia immagazzinata in un induttore}
		incremento dovuto al passaggio di corrente\quad$\mathrm dU=LI\,\mathrm dI$\\
		energia di un induttore carico\quad$U=\frac 12 LI^2$
	
\section{Equazioni di Maxwell nel vuoto}
	\begin{tabular}{@{}ll}
	\textbf{I equazione}&\textbf{II equazione}\\
	$\mathrm{div}\,\mathbf E_0(\mathbf r,t)=\frac{1}{\varepsilon_0}\varrho(\mathbf r,t)$&$\mathrm{div}\,\mathbf B_0(\mathbf r,t)=0$\\
	\textbf{III equazione}&\textbf{IV equazione}\\
	$\mathrm{rot}\,\mathbf E_0(\mathbf r,t)=-\frac{\partial\mathbf B_0(\mathbf r,t)}{\partial t}$&$\mathrm{rot}\,\mathbf B_0(\mathbf r,t)=\mu_0\mathbf J$
	\end{tabular}
...
\section{Capacità di alcuni sistemi di conduttori}
	...
	\Delta V=-\int_{R_\mathrm{i}}^{R_\mathrm{e}}\mathbf E\cdot\mathrm d\mathbf s=\frac{Q}{4\pi\varepsilon_0}\frac{R_\mathrm{e}-R_\mathrm{i}}{R_\mathrm{e}R_\mathrm{i}}\;\;
	\Longrightarrow\;\;
...
\itemm{corrente circolante\quad$i(t)=\frac{\mathrm dq(t)}{\mathrm dt}=\frac{\mathcal E}{R}e^{-\frac{t}{RC}}$}
...
\itemm{potenza sul condensatore\quad$P_C(t)=v_C(t)i(t)=P_{gen}-P_R$}
		\subsubsection{serie}
		\itemm{equazione di maglia\quad$\mathcal E-\frac{q(t)}{C}=\frac{\mathrm dq(t)}{\mathrm dt}R$}
	...
	\subsection{RL}
		\subsubsection{serie}
		\itemm{equazione di maglia\quad$\mathcal E-L\frac{\mathrm dI}{\mathrm dt}=RI$}
	\subsection{LC}
	\subsection{RLC}
