...
\section{Identità di primo ordine}
	\paragraph{gradiente di una funzione composta} $\nabla(f\circ F) = \fracp{}{x_i}\{f(F(x_1,\ldots,x_n))\}\mathbf e_i = f^\prime(F(x_1,\ldots,n_n))\fracp{}{x_i}\{F(x_1,\ldots,x_n)\}\mathbf e_i = (f^\prime\circ F) \nabla F$
		
	\paragraph{gradiente di un campo composto} $\nabla(F\circ\mathbf A) = \fracp{}{x_i}\{F(A_1(x_1,\ldots,x_n),\ldots,A_n(x_1,\ldots,x_n))\}\mathbf e_i = \ldots= (\nabla F\circ\mathbf A)\nabla \mathbf A$
	...
	\paragraph{divergenza del prodotto} $\nabla \cdot (f \mathbf A) = \fracp{}{x_i}\, \mathbf e_i \cdot A_j\, \mathbf e_j = \fracp{(f A_i)}{x_i}=f\, \fracp{A_i}{x_i} + \fracp{f}{x_i} A_i=f\, \dive{\mathbf A} + \nabla f \cdot \mathbf A$
	...
	\paragraph{laplaciano del prodotto} $\nabla^2 (f g) = \nabla \cdot \nabla(f g) = \fracp{}{x_i}\, \mathbf e_i \cdot \big( f \fracp{g}{x_j} \mathbf e_j + \fracp{f}{x_j} g\, \mathbf e_j \big) = \fracp{}{x_i} \{f \fracp{g}{x_i}\} + \fracp{}{x_i} \{\fracp{f}{x_i} g\} = f \fracpq{g}{x_i} + \fracp{f}{x_i}\fracp{g}{x_i} + \fracp{f}{x_i}\fracp{g}{x_i} + \fracpq{f}{x_i}\, g = f\, \nabla^2 g + g\, \nabla^2 f + 2 \nabla f \cdot \nabla g$

\section{Identità di secondo ordine}
	\paragraph{gradiente della divergenza} $\nabla(\nabla \cdot \mathbf A) = \fracp{}{x_i} \{\fracp{}{x_j} \mathbf e_j \cdot A_k \mathbf e_k\}\, \mathbf e_i = \fracp{}{x_i} \{\fracp{A_j}{x_j}\}\, \mathbf e_i$
	
	\paragraph{divergenza del gradiente} laplaciano
	...
	\paragraph{rotore del gradiente} $\nabla \times {(\nabla f)}=\varepsilon_{ijk} \fracp{}{x_i}\fracp{f}{x_j}\, \mathbf e_k = \varepsilon_{ijk}\frac{\partial^2 f}{\partial x_i\partial x_j}\, \mathbf e_k$.\;\; Se $f \in C^2 \Rightarrow \partial_{ij}f = \partial_{ji}f \Rightarrow \varepsilon_{ijk}\fracpp{f}{x_i}{x_j}\, \mathbf e_k = (\varepsilon_{ijk} + \varepsilon_{jik})\fracpp{f}{x_i}{x_j} \mathbf e_k = \mathbf 0$
